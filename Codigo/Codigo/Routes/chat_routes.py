...
def ver_chats(payload, idU):
    ...
    return render_template("lista_chats.html", chats=resultado, idUsuario=id_usuario)
...
def crear_chat(payload, idU):
    try:
        id_usuario = int(payload.get("sub"))
        if id_usuario != idU:
            return jsonify({"error": "No tienes permiso para crear chats para este usuario"}), 403
        ...
        return jsonify({
            ...
            "chat": {"id_chat": str(resultado.get("id_chat"))},
        }), 201
    except (TypeError, ValueError):
        return jsonify({"error": "Datos invalidos para crear el chat"}), 400
    except Exception as e:
        return jsonify({"error": f"Error al crear chat: {str(e)}"}), 500
